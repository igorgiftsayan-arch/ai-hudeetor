...
- Дополнительный targeted memory/history gate создал один безопасный `supportStrategy` fact (`receipt=1`, `factsWritten=1`). Следующий штатный bounded context: 254 Unicode characters, SHA-256 `5d10e8290684faaad95e4872ee51d968e477fc12878c8708aa722cf943892e3`, `activeFactCount=1`, `contextIncludesActiveFact=true`; request history содержала 3 сообщения (`user,assistant,user`), то есть 2 prior messages. Provider-вызов первоначально завершился локальным `outcomeUnknown`: ledger сохранил одну reservation без confirmation/refund, а UI безопасно заблокировал новый запрос.
- Audited GenAPI reconciliation выполнена для этой операции по подтверждённому provider request `54055527`. Read-only provider result строго совпал с исходным запросом по request ID, network/model, порядку и SHA-256 каждого сообщения и временному окну. Транзакционная финализация перевела operation в `succeeded`, сохранила ровно одно assistant message, одну `aiConfirmation`, usage `560/717/1277`, cost `5.23` и latency `30910 ms`; `aiRefund` отсутствует. Повтор той же команды вернул `replay=true` и не создал дубликатов. Durable audit marker и memory-extraction event существуют по одному и опубликованы; active memory fact не размножился. Blocking operations пользователя: `0`, ledger balance после двух ранее успешных запросов и reconciled reservation: `98`.
- Reconciliation PostgreSQL integration проверена на чистой PostgreSQL 17: migrations `0000–0013` применены дважды, Drizzle metadata содержит 14 записей, focused suite прошёл `1/1`. API/worker log scan после reconciliation дал `0` совпадений запрещённых raw provider/prompt/response/API-key fields. Проверка использовала код commit `e4b930f`; running Compose services и stable не пересоздавались.
...
- Оставшийся блокер полного real-AI recovery: требуется подтверждённый контракт GenAPI на идемпотентный initial submit/status-by-client-id либо отдельно принятое продуктовое правило для необратимо потерянного pre-ID результата. Post-request-ID recovery теперь проверена; pre-ID ambiguity и систематическая автоматическая reconciliation orchestration этой проверкой не закрыты.
