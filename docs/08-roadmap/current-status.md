...
- В отдельной UI-ветке от интеграционной базы `2bf0430` подготовлены не подключённые к сети client-компоненты: локальный выбор/preview/валидация фото, явное разделение analysed/confirmed с датой и временем только на этапе подтверждения, а также capability/permission UI для push. Они не создают дневник, не запрашивают browser permission без клика, не используют mock API и ожидают завершённый OpenAPI-контракт для маршрута и реальной интеграции.
