...
- Марафонный isolated runtime checkpoint принят на `912d000`: daily metrics, shared podiums, stale-task recovery, consent disclosure и mobile browser path проверены. Это не означает общий completion real AI: настоящий GenAPI выявил блокер `outcomeUnknown` — без server-side reconciliation/recovery composer остаётся заблокированным. Backend выполняет bounded diagnostics статуса/provider reference без resend или refund; до согласованного API recovery fix не менять semantics `unknown` и не обходить блокировку во frontend.
...
- Shared isolated stand `atlas-gerbi-marathon` проверяет join, wellness, captain task, daily weight, daily podiums и AI boundary без изменения stable. Browser GenAPI acceptance подтвердила disabled UI до consent, сохранение consent актуальной версии, два последовательных `succeeded` ответа и ledger delta `-2`; privacy-safe builder proof подтвердил передачу структурированного timezone/persona/weight context во второй запрос. Точная evidence-граница и image digests — в [runtime evidence](../07-deployment/gerbi-marathon-runtime-evidence.md).
- External-provider consent в UI прямо сообщает передачу сообщения и необходимого контекста в GenAPI для ответа; misleading test-mode disclosure не показывается в `genapi` mode. Версия consent evidence остаётся server-authoritative `test-v1`.
- Финальный browser checkpoint на isolated checkout `912d000` подтвердил public login/readiness `200`, mobile `390px` без overflow и безопасное обновление UI после `MARATHON_TASK_DATE_INVALID`. Targeted memory/history gate отдельно подтвердил один active safe fact в bounded context и два prior messages; следующий реальный provider-вызов получил `outcomeUnknown`, сохранил reservation без terminal effect и заблокировал composer. GenAPI reconciliation не реализована и остаётся отдельной задачей.
- Read-only provider review подтвердил native async contract Grok 4.5 с `request_id` и polling, но не нашёл документированной идемпотентности initial submit или lookup по client ID. Поэтому recovery после `request_id` технически возможен, а pre-ID timeout остаётся блокером полного real-AI acceptance до ответа GenAPI или отдельной принятой policy. Новые paid probes, adapter migration и ручные ledger-изменения не выполнялись.
