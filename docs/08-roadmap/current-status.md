...
- Дневная формула веса, baseline capture, командная агрегация wellness, podium/tie rules, реальные даты, команды и капитаны остаются `TBD`. Поэтому `dailyPercent` и podiums намеренно `null`, а pilot нельзя считать полностью настроенным для реальных участников.
