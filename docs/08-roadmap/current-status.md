...
- Shared isolated stand `atlas-gerbi-marathon` проверяет join, wellness, captain task, daily weight и AI boundary без изменения stable. Финальный shared-browser GenAPI gate пройден на synthetic account: до consent composer заблокирован, после current-version consent два последовательных реальных ответа завершились успешно; server-side ledger подтвердил две reservation/confirmation без refund. Точная evidence-граница и image digests — в [runtime evidence](../07-deployment/gerbi-marathon-runtime-evidence.md).
