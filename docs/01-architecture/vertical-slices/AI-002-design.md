...
До сопоставления текст нормализуется NFC, trim, lowercase; повторные пробелы сворачиваются. Extractor рассматривает одно простое утверждение на предложение. Sensitive/medical denylist применяется первым и отклоняет всё предложение независимо от allowlist.
