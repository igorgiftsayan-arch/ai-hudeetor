...
Герби-Марафон добавляет backend-контур `POST /api/v1/marathons`, `POST /api/v1/marathon-team-memberships`, `GET /api/v1/marathons/current`, owner-scoped `GET/PUT /api/v1/marathon-wellness-reports/{reportDate}`, captain-only `PUT /api/v1/marathon-captain-tasks/{taskDate}`, owner completion `PUT /api/v1/marathon-captain-tasks/{taskId}/completion` и безопасный командный read model `GET /api/v1/marathon-teams/current/today`. Bootstrap дополнительно требует server-side feature flag и allowlist пользователя. Все mutation routes требуют session, CSRF/Origin и `Idempotency-Key`; membership, дата и captain authority проверяются на backend. Team today возвращает дневной процент только при точных weight rows вчера/сегодня, wellness `0..8`, self-reported task completion и до трёх shared-place podium groups. API не возвращает raw weight, накопленные итоги, AI chat или memory. Точный контракт — в [GERBI-MARATHON-PILOT](../01-architecture/vertical-slices/GERBI-MARATHON-PILOT-contract.md).

Для явного provider consent пилот использует `GET/PUT /api/v1/users/me/ai-provider-consent`. Backend публикует configured provider mode, disclosure и текущую версию документа; внешний provider вызывается только при сохранённом consent этой версии. Проверка выполняется перед dispatch и повторно в worker boundary.
