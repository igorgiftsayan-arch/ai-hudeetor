...
export class AutomaticRecoveryService implements OnModuleInit {
  ...
  async sweep():Promise<void>{
    await this.database.transaction(async(client)=>{
      const retryFood=await client.query<{id:string}>(`update food_analyses a set status='queued',updated_at=now() from food_analysis_request_receipts r where r.food_analysis_id=a.id and a.status='processing' and a.updated_at < now()-interval '2 minutes' and r.submission_state='prepared' returning a.id`);
      ...
      const retryAi=await client.query<{id:string}>(`update ai_operations a set status='queued',updated_at=now() where a.status='processing' and a.updated_at < now()-interval '2 minutes' and not exists(select 1 from ai_operation_request_receipts r where r.operation_id=a.id) returning a.id`);
      ...
      const retryPreparedAi=await client.query<{id:string}>(`update ai_operations a set status='queued',updated_at=now() from ai_operation_request_receipts r where r.operation_id=a.id and a.status='processing' and a.updated_at < now()-interval '2 minutes' and r.submission_state='prepared' returning a.id`);
