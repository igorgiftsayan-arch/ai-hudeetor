...
export class FoodAnalysisProcessor {
  ...
  async process(job: Job<{outboxId:string}>): Promise<void> {
    ...
    const claimed = await this.db.transaction(async (client) => {
      ...
      if (payload && (payload.analysisId !== analysisId || payload.provider !== row.runtime_adapter)) throw new Error('Food prepared receipt configuration mismatch');
      if (!payload) {
      ...
      payload = { analysisId, provider:this.config.provider,networkId:this.config.provider==='fake'?'fake-food-v1':this.config.networkId,modelVersion:this.config.provider==='fake'?'fake-food-v1':this.config.modelVersion,promptVersion:'food-analysis-v1',objectKey: source.object_key, imageSha256: source.sha256, knownProfile: { targetWeightKg: source.target_weight_kg ?? null, facts: source.facts } };
      }
      const serialized = JSON.stringify(payload);
      const hash = row.saved_hash ?? createHash('sha256').update(serialized).digest('hex');
  ...
  private async executeVision(claimed:any):Promise<VisionResult>{
    ...
    const requestBody={is_sync:false,model:claimed.payload.modelVersion,response_format:{type:'json_object'},messages:[{role:'system',content:'Return strict JSON only. Recognize visible food without inventing hidden ingredients. Use only supplied known profile facts for suitability. If insufficient, use status insufficientData. Food/weight correlations and individual weight changes do not establish causation. Never claim a food or meal caused weight gain or loss. If food and weight data are insufficient or not temporally comparable, explicitly state that limitation and do not infer an effect on weight.'},{role:'user',content:[{type:'text',text:`Known profile JSON: ${JSON.stringify(claimed.payload.knownProfile)}. Required JSON: {recognized:{kind:food|nonFood|ambiguous,dishName:string|null,items:[{name,confidence}],uncertaintyNotes:string[]},suitability:{status:matches|doesNotMatch|mixed|insufficientData,source:profile|none,observations:string[],missingData:string[]}}`},{type:'image_url',image_url:{url:`data:${object.ContentType ?? 'image/jpeg'};base64,${bytes.toString('base64')}`}}]}]};
    try { response=await fetch(`${this.config.nativeBaseUrl.replace(/\/$/,'')}/networks/${encodeURIComponent(claimed.payload.networkId)}`,{method:'POST',headers:{Authorization:`Bearer ${this.config.apiKey}`,'Content-Type':'application/json','Accept':'application/json','X-Request-ID':claimed.id},body:JSON.stringify(requestBody),signal:controller.signal}); }
...
function extractProviderContent(value:any):string|null{const candidates=[value?.result?.[0]?.message?.content,value?.full_response?.[0]?.message?.content,value?.full_response?.choices?.[0]?.message?.content,value?.full_response?.[0]?.choices?.[0]?.message?.content,value?.result?.[0],value?.output?.choices?.[0]?.message?.content];return candidates.find((item)=>typeof item==='string'&&item.trim())??null;}
