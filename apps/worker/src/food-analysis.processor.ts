...
export class FoodAnalysisProcessor {
  ...
  constructor(@Inject(DatabaseToken) private readonly db: DatabaseService, private readonly config: {
    ...
    apiKey?:string;nativeBaseUrl:string;model?:string;timeoutMs:number;
  ...
  async process(job: Job<{outboxId:string}>): Promise<void> {
    ...
    const claimed = await this.db.transaction(async (client) => {
      ...
      const payload = { analysisId, provider:this.config.provider,model:this.config.provider==='fake'?'fake-food-v1':this.config.model,promptVersion:'food-analysis-v1',objectKey: source.object_key, imageSha256: source.sha256, knownProfile: { targetWeightKg: source.target_weight_kg ?? null, facts: source.facts } };
  ...
  private async executeVision(claimed:any):Promise<VisionResult>{
    ...
    if(!this.storage||!this.config.s3||!this.config.apiKey||!this.config.model) return {kind:'technicalError',errorCategory:'providerConfiguration'};
    ...
    const requestBody={is_sync:false,model:'gpt-4o-2024-08-06',response_format:{type:'json_object'},messages:[{role:'system',content:'Return strict JSON only. Recognize visible food without inventing hidden ingredients. Use only supplied known profile facts for suitability. If insufficient, use status insufficientData.'},{role:'user',content:[{type:'text',text:`Known profile JSON: ${JSON.stringify(claimed.payload.knownProfile)}. Required JSON: {recognized:{kind:food|nonFood|ambiguous,dishName:string|null,items:[{name,confidence}],uncertaintyNotes:string[]},suitability:{status:matches|doesNotMatch|mixed|insufficientData,source:profile|none,observations:string[],missingData:string[]}}`},{type:'image_url',image_url:{url:`data:${object.ContentType ?? 'image/jpeg'};base64,${bytes.toString('base64')}`}}]}]};
    try { response=await fetch(`${this.config.nativeBaseUrl.replace(/\/$/,'')}/networks/${encodeURIComponent(this.config.model)}`,{method:'POST',headers:{Authorization:`Bearer ${this.config.apiKey}`,'Content-Type':'application/json','Accept':'application/json','X-Request-ID':claimed.id},body:JSON.stringify(requestBody),signal:controller.signal}); }
    catch(error){ clearTimeout(timer); return error instanceof DOMException&&error.name==='AbortError'?{kind:'outcomeUnknown'}:{kind:'technicalError',errorCategory:'providerUnavailable'}; }
    clearTimeout(timer);
    ...
    while(!controller.signal.aborted){
      ...
      let poll:Response; try{poll=await fetch(`${this.config.nativeBaseUrl.replace(/\/$/,'')}/request/get/${encodeURIComponent(providerReference)}`,{headers:{Authorization:`Bearer ${this.config.apiKey}`,'Accept':'application/json'},signal:controller.signal});}catch(error){if(controller.signal.aborted){clearTimeout(timer);return{kind:'outcomeUnknown',providerReference} as VisionResult;}continue;}
      if(!poll.ok){if(poll.status===404)continue;clearTimeout(timer);return{kind:'technicalError',errorCategory:`pollHttp${poll.status}`};}
